...
In addition to the |\abbrev| command, there are a few other commands to change the format of the the |\listofabbreviations| as shown in Table~\ref{abbrev}. Note that the Key to Abbreviations written to the auxiliary file with extension |.lob| which can be removed along with other |.aux| files if needed. 
