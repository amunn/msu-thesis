...
If you need to add an appendix or appendices to a chapter, must use the \pkg{[chapterapp]} class option. The code for the appendices themselves is  exactly as described above.

\subsection{Bibliography spacing}\label{bibspacing}
Please note that although bibliography entries must be single spaced, there must be a space between each entry. This parameter is set by your bibliography package and not by the class itself. If you are using \pkg{natbib} or \pkg{biblatex} these spacing parameters are set for you. If you are using another package for your bibliography you are responsible for adjusting the relevant spacing parameter.
